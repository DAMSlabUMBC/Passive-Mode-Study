...
pathlist = Path(r"File_Name").glob('**/*.pcap') # go through a folder of .pcaps
for path in pathlist: # iterate through each path
    ...
    for i in cap_file:
        ...
        conversations[endpoint]["total_packet_count"] += 1
        conversations[endpoint]["total_byte_count"] += int(i.length)
