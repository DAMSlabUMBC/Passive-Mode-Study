        ...
        for path in paths:
            ...
            with open(file_location, newline='') as infile:
                ...
                for row in file_reader:
                    ...
                    ip = row[0]
                    unmodified_hostname = row[1]
                    modified_hostname = row[2]
                    ip_geo = row[3]
                    cert_geo = row[4]
                    packet_count = int(row[5])
                    byte_count = int(row[6])
                    txpacket_count = int(row[7])
                    txbyte_count = int(row[8])
                    rxpacket_count = int(row[9])
                    rxbyte_count = int(row[10])
                    ...
                    if modified_hostname == "None":
                        out_row = [[ip], modified_hostname, ip_geo, cert_geo, packet_count, byte_count, txpacket_count, txbyte_count, rxpacket_count, rxbyte_count]
                    ...
                    # Otherwise, we process for merges and updates
                        ...
                        for known_host in all_hosts_in_file:
                            ...
                            if modified_hostname == known_host[1]:
